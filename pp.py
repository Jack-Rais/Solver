...
def __get_adjacency(rect1, rect2, allow_distance):
...
def __create_middle_nodes(graph:nx.Graph) -> nx.Graph:
    ...
    for edge in graph.edges():
        ...
        is_there, contact, type_contact = __get_adjacency(
...
def modify(graph:nx.Graph):
    ...
    new_graph = __create_middle_nodes(graph)
    end_graph = nx.Graph()
            ...
            if len(list(x := new_graph.edges(other_edge))) <= 2:
                ...
                for edge_other in x:
                    ...
                    if not end_graph.has_edge(other_edge, other_edge_other):
                        end_graph.add_edge(other_edge, other_edge_other)

                    if not end_graph.has_edge(node[0], other_edge_other):
                        end_graph.add_edge(node[0], other_edge_other)
...
for node in graph.nodes(data = True):
    ...
    xstart, ystart, xend, yend = normalize(node[1]['node'].pos)

    plt.plot(
        [xstart, xend, xend, xstart, xstart],
...
for edge in new.edges():
    ...
    center1 = new.nodes[edge[0]]['center']
    center2 = new.nodes[edge[1]]['center']
